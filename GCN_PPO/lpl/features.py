...
def yosys_stats(design_file, yosys_binary, stats):
    ...
    try:
        ...
        if 'nots' not in stats:
            stats['nots'] = 0.0
        if 'Number of wires' not in stats:
            stats['number_of_wires'] = 0.0
        if 'Number of public wires' not in stats:
            stats['number_of_public_wires'] = 0.0
...
def extract_features(design_file, yosys_binary='yosys', abc_binary='abc'):
    ...
    # (6) - number wires and public wires
    features['number_of_wires'] = stats['number_of_wires']
    features['Number of public wires'] = stats['number_of_public_wires']
    ...
    return features['number_of_nodes'], features['number_of_levels']