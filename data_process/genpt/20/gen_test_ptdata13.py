...
def preprocessGraphData(aig_graphFiles,processedDir,desName,synthID2VecDict, id):
    ...
    data.synID = [synID]
    #data.stepID = [stepID]
    data = test_addNormalizedTargets(data, numGatesAndLPStats, meanVarTargetDict)
...
class NetlistGraphDataset(Dataset):
    ...
    def processed_file_names(self):
        processedFiles = []
        for i in range(135000+32500, 200000):
    ...
    def process(self):
        ...
        Parallel(n_jobs=10)(delayed(preprocessGraphData)(aig_graphFiles,self.processed_dir,self.des,synthID2VecDict, id) for id in range(135000+32500, 200000))
...
def main():
    ...
    for des in test_design:
        design_name = des
        design_root = os.path.join("/home/yangch/TODAES/data_set/ptdata/test/", des)
