...
def preprocessGraphData(aig_graphFiles,processedDir,desName,synthID2VecDict, id):
    ...
    data.synID = [synID]
    #data.stepID = [stepID]
    data = valid_addNormalizedTargets(data, numGatesAndLPStats, meanVarTargetDict)
...
class NetlistGraphDataset(Dataset):
    ...
    def process(self):
        ...
        Parallel(n_jobs=10)(delayed(preprocessGraphData)(aig_graphFiles,self.processed_dir,self.des,synthID2VecDict, id) for id in range(215000, 217500))
...
def main():
    ...
    for des in valid_design:
        design_name = des
        design_root = os.path.join("/home/yangch/TODAES/data_set/ptdata/valid/", des)
