...
from environment import LogicSession as Env
...
def testReinforce(filename, ben, options):
    now = datetime.now()
    dateTime = now.strftime("%m/%d/%Y, %H:%M:%S") + "\n"
    print("Time ", dateTime)
    env = Env(options, filename)
    #vApprox = Linear(env.dimState(), env.numActions())
    vApprox = RF.PiApprox(env.state_dim, env.action_dim, 8e-4, RF.FcModelGraph)
    baseline = RF.Baseline(0)
    vbaseline = RF.BaselineVApprox(env.state_dim, 3e-3, RF.FcModel)
...
if __name__ == "__main__":
    ...
    """
    testReinforce('/home/yangch/benchmark/bench_test/i10.aig', "i10", options)
    """
