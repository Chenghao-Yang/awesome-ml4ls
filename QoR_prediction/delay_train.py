...
def train(model,device,dataloader,optimizer):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc="Iteration",file=sys.stdout)):
        ...
        lbl = batch.target_delay.reshape(-1, 1)
        optimizer.zero_grad()
        pred = model(batch)
...
def main():
    ...
    train_dl = DataLoader(trainDS,shuffle=True,batch_size=batchSize,pin_memory=True,num_workers=20)
    valid_dl = DataLoader(validDS,shuffle=True,batch_size=batchSize,pin_memory=True,num_workers=20)
    test_dl = DataLoader(testDS,shuffle=True,batch_size=batchSize,pin_memory=True,num_workers=20)
